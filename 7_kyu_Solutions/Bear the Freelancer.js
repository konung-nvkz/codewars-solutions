...
function bearDollars(jobs) {
  return jobs.reduce((totalAmount, [hours, client]) => totalAmount + hours * (+rates[client.toLowerCase()] || 125), 0);
}

// Example usage
const jobs = [[10, 'Close Friend'], [3, 'Acquaintance'], [7, 'Lead from web'], [6, 'Friend'], [2, 'From advertisements']];
const invoiceAmount = bearDollars(jobs);
console.log(invoiceAmount); // Output: 1975