...
function FindFactors(num) {
    ...
    function relatively_prime(n, arr) {
        let relativelyPrimeNumbers = [];
        let nFactors = FindFactors(n);
        
        for (let i = 0; i < arr.length; i++) {
        let numFactors = FindFactors(arr[i]);
        let hasCommonFactor = false;
        
        for (let j = 0; j < numFactors.length; j++) {
            if (nFactors.includes(numFactors[j])) {
        ...
        if (!hasCommonFactor) {
            relativelyPrimeNumbers.push(arr[i]);
        }
        }
    ...
    //Tests
    console.log(relatively_prime(8, [1, 2, 3, 4, 5, 6, 7]));
    // Output: [1, 3, 5, 7]
    
    console.log(relatively_prime(15, [72, 27, 32, 61, 77, 11, 40]));
    // Output: [32, 61, 77, 11]
    
    console.log(relatively_prime(210, [15, 100, 2222222, 6, 4, 12369, 99]));
    // Output: [15, 100, 2222222, 6, 4, 12369, 99]