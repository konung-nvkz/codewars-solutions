...
the Triple Shiftian are so defined: T[n] = 4 * T[n-1] - 5 * T[n-2] + 3 * T[n-3].
...
'
Special thanks to the first person I met in person here in London just because of 
CW and that assisted me during the creation of this kata ;)
*/
...
// we start from the last element of array base (base[i])
// count the next element T[n] = 4 * T[n-1] - 5 * T[n-2] + 3 * T[n-3].
// and push it to array
// when i becomesequal to n, we finish and return the last element of the array

console.log(tripleShiftian([6,7,2],26));// == -2575238999
...
console.log(tripleShiftian([1,9,2],2));// ==  2
